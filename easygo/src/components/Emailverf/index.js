...
import { Container, FormContent, FormWrap, FormTop, Icon, ExitIcon, Form, FormH1, FormLabel, FormInput, FormButton,NavButton, Text } from './Emailelements'
...
const Emailverf = () => {
  ...
  const{user,getOtp,fotpVerf}=context;
 ...
 const handleSubmit=async(e)=>{
   ...
   //console.log({email:cred.email})
    ...
    const handleClick=async(e)=>{
        ...
        await fotpVerf(cred.email,enote.otp);
        const token =localStorage.getItem('fotp');
        console.log(token);
        if(token!=null){
